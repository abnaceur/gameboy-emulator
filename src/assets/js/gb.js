...
window.gb = function (file, canvas, options) {
	...
	loadbios.open("GET", options.rootDir + "/assets/dmgbios.bin");
	loadbios.responseType = "arraybuffer";
	loadbios.send();
	console.log("isDefaultLoaded :", isDefaultLoaded);
	loadbios.onload = function () {
		if (loadbios.status == 200) bios = new Uint8Array(loadbios.response);
		biosLoaded++;
		if (gameLoaded && (biosLoaded == 2) && isDefaultLoaded) init();
	}
